...
async def async_setup_entry(
    ...
    if core.alarm is not None and core.locate is None:
...
class AutoMopDrySwitch(CoordinatorEntity[XiaomiVacuumCoordinator], SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        await self.hass.async_add_executor_job(self.coordinator.device.set_auto_mop_dry, True)
        await self.coordinator.async_request_refresh()

    async def async_turn_off(self, **kwargs) -> None:
        await self.hass.async_add_executor_job(self.coordinator.device.set_auto_mop_dry, False)
        await self.coordinator.async_request_refresh()


class MopDryingSwitch(CoordinatorEntity[XiaomiVacuumCoordinator], SwitchEntity):
    """Start or stop the base's current mop-drying operation."""
